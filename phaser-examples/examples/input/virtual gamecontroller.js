...
function preload() {
    ...
    game.load.spritesheet('mario', 'phaser-examples/assets/misc/mariospritesheet-small.png',50,50); // key, sourcefile, framesize x, framesize y
    //background, ground, fireball images
    game.load.image('ground', 'phaser-examples/assets/misc/2048x48-ground.png');
    game.load.image('clouds', 'phaser-examples/assets/misc/clouds.jpg');
    game.load.image('fireball', 'phaser-examples/assets/misc/fireball.png',40,30);
    //gamepad buttons
    game.load.spritesheet('buttonvertical', 'phaser-examples/assets/buttons/buttons-big/button-vertical.png',64,64);
    game.load.spritesheet('buttonhorizontal', 'phaser-examples/assets/buttons/buttons-big/button-horizontal.png',96,64);
    game.load.spritesheet('buttondiagonal', 'phaser-examples/assets/buttons/buttons-big/button-diagonal.png',64,64);
    game.load.spritesheet('buttonfire', 'phaser-examples/assets/buttons/buttons-big/button-round-a.png',96,96);
    game.load.spritesheet('buttonjump', 'phaser-examples/assets/buttons/buttons-big/button-round-b.png',96,96);
    // fullscreen setup
...
function create() {
    ...
    game.physics.p2.enable(ground, true);  //enable physics so our player will not fall through ground but collide with it
    ...
    // ***************************************************************************************************************************************
    // phaser-p2Helper
    var shapesCont_collATBT = new OnContact(game, this);
    // create and pass your functions
    shapesCont_collATBT.create(
        // timeObject
        {
            top: {},
            right: {},
            bottom: {
                delayed: new Map([['onBegin', 0], ['onEnd', 250]])
            },
            left: {},
            on: {}
        },
        // pushCheckFunctionsMap
        new Map([
            ['top', function (body, shapeA, shapeB, shapeB_material_name, equation) {return equation[0].contactPointA[1] > 0.75;}],
            ['right', function (body, shapeA, shapeB, shapeB_material_name, equation) {return equation[0].contactPointA[0] < -0.75;}],
            ['bottom', function (body, shapeA, shapeB, shapeB_material_name, equation) {return equation[0].contactPointA[1] < -0.75;}],
            ['left', function (body, shapeA, shapeB, shapeB_material_name, equation) {return equation[0].contactPointA[0] > 0.75;}],
            ['on', function (body, shapeA, shapeB, shapeB_material_name, equation) {return true;}]
        ]),
        // pushEventFunctionsMap
        new Map([['top', function (body, shapeA, shapeB, equation, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - pushed - ' + arrayName);
        }], ['right', function (body, shapeA, shapeB, equation, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - pushed - ' + arrayName);
        }], ['bottom', function (body, shapeA, shapeB, equation, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - pushed - ' + arrayName);
        }], ['left', function (body, shapeA, shapeB, equation, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - pushed - ' + arrayName);
        }], ['on', function (body, shapeA, shapeB, equation, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - pushed - ' + arrayName);
        }]]),
        // spliceEventFunctionsMap
        new Map([['top', function (body, shapeA, shapeB, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - spliced - ' + arrayName);
        }], ['right', function (body, shapeA, shapeB, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - spliced - ' + arrayName);
        }], ['bottom', function (body, shapeA, shapeB, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - spliced - ' + arrayName);
        }], ['left', function (body, shapeA, shapeB, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - spliced - ' + arrayName);
        }], ['on', function (body, shapeA, shapeB, array, direction, arrayName) {
            console.log(direction + ' - id: ' + shapeB.id + ' - spliced - ' + arrayName);
        }]]),
        // addRealtime
        true
    );
    // attach pushShape and spliceShape function to player body onBeginContact and onEndContact signal (can be done manually to finegrain/prefilter)
    shapesCont_collATBT.attach(player);
    console.log(shapesCont_collATBT);
    // ***************************************************************************************************************************************
    // adding extra stuff to the stage
    var ground1 = game.add.sprite(game.world.width/2, game.world.height-30,'ground');
    game.physics.p2.enable(ground1, true);
    ground1.body.static=true;
    var ground2 = game.add.sprite(350, game.world.height-200,'ground');
    ground2.width = 200;
    game.physics.p2.enable(ground2, true);
