...
var MasterOnContact = (function () {
    ...
    _createClass(MasterOnContact, [{
        ...
        key: 'onBeginContact',
        value: function onBeginContact(body, shapeA, shapeB, equation) {
            this.pushShape(body, shapeA, shapeB, equation);
        }
    }, {
        key: 'onEndContact',
        value: function onEndContact(body, shapeA, shapeB) {
            this.spliceShape(body, shapeA, shapeB);
        }
    }, {
        key: 'pushShape',
        value: function pushShape(body, shapeA, shapeB, equation) {
            var _this2 = this;
                ...
                directions.forEach(function (direction) {
                    if (!arrayName) {
                        for (var key in _this2.shapeCont[direction]) {
                            _this2._pushShape(body, shapeA, shapeB, equation, direction, key);
                        }
                    } else {
                        _this2._pushShape(body, shapeA, shapeB, equation, direction, arrayName);
                    }
                });
            }
        }
        ...
        key: '_pushShape',
        value: function _pushShape(body, shapeA, shapeB, equation, direction, arrayName) {
            var _this3 = this;
        ...
        key: 'spliceShape',
        value: function spliceShape(body, shapeA, shapeB) {
            var _this4 = this;

            var arrayName = arguments[3] === undefined ? false : arguments[3];
                        ...
                        if (directionResult.indexOf(direction) === -1) {
                            ...
                            if (!arrayName) {
                                for (var key in _this4.shapeCont[direction]) {
                                    _this4._spliceShape(body, shapeA, shapeB, direction, key);
                                }
                            } else {
                                _this4._spliceShape(body, shapeA, shapeB, direction, arrayName);
                            }
                        }
                    });
        ...
        key: '_spliceShape',
        value: function _spliceShape(body, shapeA, shapeB, direction, arrayName) {
            var _this5 = this;
        ...
        key: '_push',
        value: function _push(body, shapeA, shapeB, equation, direction, arrayName) {
        ...
        key: '_splice',
        value: function _splice(body, shapeA, shapeB, direction, arrayName) {
...
var OnContact = (function (_MasterOnContact) {
	...
	function OnContact(game, parent) {
		...
		this.pushCheckFunctions = new Map([['top', function (body, shapeA, shapeB, shapeB_material_name, equation) {
			return equation[0].contactPointA[1] > _this.minPointContact;
		}], ['right', function (body, shapeA, shapeB, shapeB_material_name, equation) {
			return equation[0].contactPointA[0] < -_this.minPointContact;
		}], ['bottom', function (body, shapeA, shapeB, shapeB_material_name, equation) {
			return equation[0].contactPointA[1] < -_this.minPointContact;
		}], ['left', function (body, shapeA, shapeB, shapeB_material_name, equation) {
			return equation[0].contactPointA[0] > _this.minPointContact;
		}], ['on', function (body, shapeA, shapeB, shapeB_material_name, equation) {
			return true;
		}]]);
		// functions must be represented by an object with the same key inside this.time to take effect
		this.pushEventFunctions = new Map([['top', function (body, shapeA, shapeB, equation, array, direction, arrayName) {
		...
		// functions must be represented by an object with the same key inside this.time to take effect
		this.spliceEventFunctions = new Map([['top', function (body, shapeA, shapeB, array, direction, arrayName) {
